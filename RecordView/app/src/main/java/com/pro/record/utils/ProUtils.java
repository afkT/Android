...
public class ProUtils {
	 ...
	 * @return
	 */
	public static String secToTimeSecondRetain(int time){
		try {
			if(time <= 0){
				return "";
			} else {
			...
			if(time <= 0){
				...
				if(time < minute){ // 小于1分钟
					...
					if(rSecond == 0){
						...
						return dHour + "h:" + rSecond + "s";
					}
					return dHour + "h:" + dMinute + "m:" + rSecond + "s";
				} else {
					// 多余的时间，直接格式化
					// 大于一天的情况
					return "1day";
